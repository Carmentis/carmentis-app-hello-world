...
async function isChanged$1(org) {
  let [ keyChange, descChange, serverChange ] = await getChanges$1(org);
...
async function publish$1(org, getGas$1) {
  ...
  if(keyChange) {
    await obj.setPublicKey({ publicKey: fromHexa(org.publicKey.publicKey) });
  }
...
async function getChanges$1(org) {
  ...
  if(org.meta.carmentisId) {
    let previousKey = await getPublicKey$2(fromHexa(org.meta.carmentisId)),
        previousDesc = await getDescription$1(fromHexa(org.meta.carmentisId)),
...
//const ENDPOINT = "http://operator.acme.me";
...
//}
  ...
  // search if an operator endpoint has been specified, otherwise takes the testapps.carmentis.io as default
  let operator_endpoint = arg.operatorURL ? arg.operatorURL : ENDPOINT$1;
  console.log(`Carmentis.wallet.request: Operator endpoint: ${operator_endpoint}`);


  return await new Promise(function(resolve, reject) {
    let socket = getSocket(operator_endpoint, connect, async (...arg) => dataCallback$1(socket, ...arg));
...
function attemptToReconnect() {
  ...
  let socket = getSocket(connectionState.endpoint, connect, async (...arg) => dataCallback(socket, ...arg));
